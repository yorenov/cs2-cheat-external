...
ImColor RGBA2ImColor(RGBA rgba)
{
	ImVec4 col(rgba.R / 255.0f, rgba.G / 255.0f, rgba.B / 255.0f, rgba.A / 255.0f);
	return ImColor(col);
}
...
void DrawEspLine(Vector3 s_pos, ImColor color, float thickness = 1.0f, bool inDown = true)
{
	auto drawlist = ImGui::GetOverlayDrawList();

	ImVec2 start = ImVec2(Process::WindowWidth / 2, (inDown ? 0 : Process::WindowHeight));
	ImVec2 end = ImVec2(s_pos.x, s_pos.y);
